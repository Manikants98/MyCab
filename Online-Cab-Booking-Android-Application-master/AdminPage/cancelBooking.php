<?php
	...
	if ($result) {
		$cancelCabQuery="UPDATE `rides` SET `stats` = 'canceled' WHERE `rides`.`ride_id` = '$ride_id'";
 		...
 		if ($result2 && $result3) {
			$response["error"] = TRUE;
            $response["error_msg"] = "Booking Canceled Successfully";
            echo json_encode($response);
		}
		else
		{
			$response["error"] = TRUE;
            $response["error_msg"] = "1: Unable to cancel ride!!";
            echo json_encode($response);
		}
	...
	{
			$response["error"] = TRUE;
            $response["error_msg"] = "2: Unable to cancel ride!!";
            echo json_encode($response);
	}
