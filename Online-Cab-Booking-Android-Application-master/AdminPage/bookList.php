...
$response = array("error" => FALSE);
    ...
    $email=$_POST['email'];
         ...
         //creating a query
 $stmt = $con->prepare("SELECT driver_email,user_email,src_lat,src_lng,dest_lat,dest_lng,date,time,carType,stats FROM rides where driver_email='$email'");

 //executing the query
 $stmt->execute();

 //binding results to the query
 $stmt->bind_result($d_email,$u_email,$srclat,$srclon,$destlat,$destlon,$date,$time,$cartype,$stat);
